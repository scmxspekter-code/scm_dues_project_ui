...
export const Defaulters: React.FC = () => {
  ...
  const [selectedDefaulter, setSelectedDefaulter] = React.useState<Member | null>(null);

  const handleExport = () => {
    const headers = "ID,Name,Phone,Amount Due,Joined Date\n";
    const rows = mockDefaulters.map(d => `${d.id},${d.name},${d.phone},${d.amountDue},${d.joinedDate}`).join("\n");
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Defaulter Info</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredDefaulters.map((defaulter) => (
                <tr key={defaulter.id} className="hover:bg-slate-50/80 transition-all group">
                  <td className="px-8 py-5">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-slate-400 font-medium">{defaulter.phone}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-5 text-center">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${defaulter.reminderHistory?.length ? 'bg-amber-50 text-amber-600' : 'bg-slate-50 text-slate-400'}`}>
                      {defaulter.reminderHistory?.length || 0} Sent
                    </span>
                  </td>
                  <td className="px-8 py-5">
                    <div className="flex flex-col">
                      <span className="font-extrabold text-slate-900 text-lg">₦{defaulter.amountDue.toLocaleString()}</span>
