    ...
    reminderHistory: [
      {
        id: 'h1',
        date: '2025-02-01',
        content: 'Gentle reminder for your annual dues.',
        status: 'Delivered',
        type: 'WhatsApp',
      },
      {
...
export const Debtors: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
              ...
              {filteredDebtors.map((debtor) => (
                <tr key={debtor.id} className="hover:bg-slate-50/80 transition-all group">
                  ...
                  <td className="px-8 py-5 text-center">
                    <span
                      className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${debtor.reminderHistory?.length ? 'bg-amber-50 text-amber-600' : 'bg-slate-50 text-slate-400'}`}
                    >
      ...
      {selectedDebtor && (
        <DebtorActionModal member={selectedDebtor} onClose={() => setSelectedDebtor(null)} />
      )}
    </div>
  );
};
